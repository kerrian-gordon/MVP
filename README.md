...
| 4. Morning digest | [`scenario-4-morning-digest/`](scenario-4-morning-digest/) | ✅ Built — tested MVP |
...
- Scenario 4's MVP (`scenario4-digest-mvp.html`, sample-data toggle for "Typical day" / "Quiet day") exercises the digest-building logic client-side against sample Tracker data. Wire it into a real Apps Script trigger (`postMorningDigest()`) that runs against a live `Tracker` tab and posts to a real Slack webhook — a prior draft exists in git history (see `docs/blueprint.md`) and can be restored or rebuilt as the basis for that.

Scenarios 1–4 are built and tested. Remaining work is wiring all four into their live automation targets (Make modules for 1–3, Apps Script + Slack webhook for 4).
